const addField = () => {
  const extraFieldDiv = document.getElementsByClassName(
    "extra-fields-div"
  )[0] as HTMLDivElement;
...
const addExtraImageField = (e: MouseEvent) => {
  ...
  currrentFieldImagesDiv.insertAdjacentHTML(
    "beforeend",
    singleImageDivHtmlCode
  );
};

const addExtraImageToFieldFunction = () => {
  const addAnotherImageButton = document.getElementsByClassName(
    "add-another-image-button-selector"
  );
  for (let i = 0; i < addAnotherImageButton.length; i++) {
    let button = addAnotherImageButton[i] as HTMLButtonElement;
