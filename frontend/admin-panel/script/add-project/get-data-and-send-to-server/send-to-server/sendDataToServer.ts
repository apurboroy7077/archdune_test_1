...
import wait from "../../../../../script/utils/custom-functions/wait";
import addAdditionalFieldsToFormData from "./add-additional-fields-to-formdata/addAdditionalFieldsToFormData.js";
import {
  showAddingButton,
  showFailedButton,
  showSuccessButton,
} from "./show-buttons/showButtons.js";

const sendDataToServer = (data: ProjectDataTypeOfSendingToServer1) => {
  const {
    additionalFields,
    basement,
    buildArea,
  ...
  myFormData.append("adminToken", giveAdminToken() as string);
  const formDataAfterAddingAdditionalFields = addAdditionalFieldsToFormData(
    myFormData,
    data
  );
  showAddingButton();
  ...
  ar7Fetch
    .postForm(
      `${SERVER_URL + ARCHDUNE_UPLOAD_NEW_PROJECT_DATA_API}`,
      formDataAfterAddingAdditionalFields
    )
    .then((response) => {
      showSuccessButton();
