...
const filteringButtons = document.getElementsByClassName("photography-filtering-button");
const imagesList = document.getElementsByClassName("images-list-of-photography")[0];
const searchInput = document.getElementsByClassName("search-input-photography-js-selector")[0];
const filteringButtonDiv = document.getElementsByClassName("photography-filtering-button-div-js-selector")[0];
const galleryDiv = document.getElementsByClassName("gallery-div-js-selector")[0];
const loadingDiv = document.getElementsByClassName("loading-div-js-selector")[0];
let serial = 1;
const giveAuthorDetailsOfData = (datas) => {
    const authorDetails = [];
...
const giveArrayOfMatchedData = (typenWords) => {
    const matchedDatas = [];
    for (let i = 0; i < imageDatas.length; i++) {
        const data = imageDatas[i];
        if (data.authorName.toLowerCase().includes(typenWords) ||
...
const makeFilteringButtons = (authorDetails) => {
...
const loadMatchingImages = (datas) => {
    let imageCodes = ``;
    for (let i = 0; i < datas.length; i++) {
        const data = datas[i];
        const imageCode = `<a
...
const loadFilteredData = (authorName) => {
...
const filteringFunction = () => {
    for (let i = 0; i < filteringButtons.length; i++) {
        ...
        const authorName = filteringButton.getAttribute("data-name");
...
const searchFunctions = () => {
    const searchingFunction1 = async (typenWords) => {
    ...
    const searchingFunction2FindAndShowResults = (typenWords) => {
        const matchedDatas = [];
        for (let i = 0; i < imageDatas.length; i++) {
            const data = imageDatas[i];
            if (data.authorName.includes(typenWords) ||
    ...
    const searchingFunction3LoadResults = (matchedDatas) => {
...
const searchFunction2 = () => {
    const searchFunction1 = (typenWords) => {
